...
(function(){
  ...
  var FS_KEY='poem-fs', TH_KEY='poem-theme', PG_KEY='poem-paged';
  var HL_KEY='poem-highlights', BM_KEY='poem-bookmarks';
  var GAP=52;
  ...
  document.getElementById('c-cup').addEventListener('click',function(){
    var last=localStorage.getItem('poem-last')||JSON.stringify({file:file,page:(paged?cur+1:1),title:document.title,ts:Date.now()});
    showHint('☁ جارٍ الحفظ…');
  ...
  document.getElementById('c-cdn').addEventListener('click',function(){
    ...
    fetch(SYNC,{cache:'no-store'}).then(function(r){return r.json();}).then(function(p){
      if(p&&p.file){ location.href='/poetry-daily/'+p.file+'?pg='+(p.page||1); }
  ...
  function saveLast(page){ try{ localStorage.setItem('poem-last', JSON.stringify(
